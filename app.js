...
readButton.addEventListener('click', async () => {
  ...
  status.textContent = 'Locating the physical card and first player name, then applying the v3.9 downward Y calibration while keeping the v3.2 X positions unchanged...';
  try {
...
function renderGeometryDiagnostics(payload) {
  ...
  geometryDiagnosticMeta.textContent =
    ...
    `Name center Y: ${nameCenterY} · Y offset: ${yOffset} · Name center on warped card: ${transformedNameCenterY} · Row scan: ${JSON.stringify(payload?.debug?.rowScan)} · Crop center Y: ${rowCenterY}`;
